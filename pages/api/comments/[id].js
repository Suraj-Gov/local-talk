  ...
  switch (method) {
    case "GET":
      try {
        ...
        const post_comments = await knex.raw(
          "SELECT json_agg(c) AS comment, json_agg(u2) as user_details, json_agg(u) AS upvotes FROM comments c LEFT JOIN upvoted u ON c.comment_id = u.upvoted_comment INNER JOIN posts p ON c.comment_post = p.post_id LEFT JOIN users u2 ON comment_author = u2.user_id WHERE post_id = (?) GROUP BY comment_id, u2",
          [post_id]
        );
        console.log(
          post_comments.rows,
    ...
    case "DELETE":
      try {
        const comment_id = req.query.id;
        const deleteComment = await knex.raw(
          "DELETE FROM comments WHERE comment_id = (?) RETURNING *",
          [comment_id]
        );
        console.log(deleteComment, `deleted comment: ${comment_id}`);
