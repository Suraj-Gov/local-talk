  ...
  switch (method) {
    case "GET":
      try {
        ...
        const getUser = await knex.raw(
          "SELECT * FROM users WHERE auth0_id = (?)",
          [auth0_id]
        );
        if (req.query.all === "1") {
          const getPosts = await knex.raw(
            "select * from users inner join posts p on users.user_id = p.post_author inner join locations l on p.post_location = l.location_id where auth0_id=(?)",
            [auth0_id]
          );
          const getComments = await knex.raw(
            "SELECT * FROM users INNER JOIN comments c ON users.user_id = c.comment_author  WHERE auth0_id=(?)",
            [auth0_id]
          );
    ...
    case "PUT":
      try {
        ...
        const updateUser = await knex.raw(
          upvote
            ? "UPDATE users SET user_points = (user_points + 1) WHERE user_id = (?) RETURNING *"
            : "UPDATE users SET user_points = (user_points - 1) WHERE user_id = (?) RETURNING *",
          [id]
        );
        console.log(
    ...
    case "DELETE":
      try {
        const id = req.query.id;
        const deleteUser = await knex.raw(
